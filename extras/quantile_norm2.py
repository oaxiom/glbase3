...
def rawsort(ifile,sort_ofile,gfile,format,step=10,suppress=False,buffer=None):
    ...
    if buffer!=None:
        cmd='sort -r -n -s -k1 -k2 -o '+sort_ofile+' --buffer-size '+str(buffer)+' --temporary-directory '+str(temp)+' '+raw_ofile
    else:
        cmd='sort -r -n -s -k1 -k2 -o '+sort_ofile+' --temporary-directory '+str(temp)+' '+raw_ofile
...
def changevalue(ifile,ref,ofile,gfile,step=10,suppress=False,buffer=None):
    ...
    if n>0:
        print 'Warning: the input genome size is smaller than the reference genome size by',n,'wiggle steps!'
...
def qnorwig(paths,gfile,out_dir='wiq_result',ref=None,iformat='wig',rformat='wig',isorted=False,rsorted=False,step=10,suppress=False,buffer=1000000,format_only=False):
    ...
    if ref==None:
        ...
        refquantile(paths=':'.join(sfiles.keys()),ofile=refpath,gfile=gfile)
    else:
...
def wig2wiq(command='wiq'):
    '''
    Description:
        This function parses input parameters, calls and passes all parameters values to the function qnorwig, 
        or print some help messages if required.
    parameters:
        none  
    '''
