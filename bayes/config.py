...
def write(filename, include_defaults=False):
    ...
    config = ConfigParser()
    params = list(_parameters.values()) if include_defaults \
                                  else [p for p in list(_parameters.values()) if p.source]
...
def parameters(section=None):
    ...
    if section:
        return [p for p in list(_parameters.values()) if p.name.startswith(section)]
    else:
        return list(_parameters.values())
